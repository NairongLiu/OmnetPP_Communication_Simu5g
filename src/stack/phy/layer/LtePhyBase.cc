...
void LtePhyBase::initialize(int stage)
{
    ...
    if (stage == inet::INITSTAGE_LOCAL)
    {
        ...
        //scheduleAt(simTime() + 0.1, LtePhyTimer);
        if (hasPar("readUeTxPowerFile") && par("readUeTxPowerFile")&&false)
            {
...
void LtePhyBase::handleMessage(cMessage* msg)
{
    ...
    if (hasPar("readUeTxPowerFile") && par("readUeTxPowerFile") && isTimerDeleted)
    {
        std::string power = DataStorage::getReceivedData("ueTxPower");
        //std::cout << "power value received: " << power << std::endl;
        if (!power.empty()) {
...
void LtePhyBase::ueTxPowerUpdate(double newPower)
{
    ueTxPower_ = newPower;
    txPower_ = newPower;
                                    std::ofstream outputFile("powertestoutput.txt", std::ios::trunc);
                                    if (outputFile.is_open()) {
                                        outputFile << ueTxPower_ << std::endl;
                                        outputFile.close();
                                    }
}

double LtePhyBase::readUeTxPowerFromFile(const char *filename)
{
