        ...
        for (Codeword cw = 0; cw < MAX_CODEWORDS; ++cw)
        {
            if (processes_[i]->isCorrect(cw))
            {
                ...
                unsigned int size = pktTemp->getByteLength();
                                ...
                                std::ofstream outputFile("mac_delay_output.txt", std::ios::trunc);
                                if (outputFile.is_open()) {
                                    outputFile << uInfo << std::endl;
                                    outputFile.close();
                                } else {
                                    EV << "Unable to open file for writing." << endl;
                                }

                                StatsLogger::logMacDelayToFile(delay);


                                //NamedPipeServer server("\\\\.\\pipe\\MyNamedPipe");
                                //server.waitForClient();
                                //std::string data = "Hello from OMNeT++";
                                //server.writeData(data);

                                //const std::string name = "SharedMemory_MacDelayUL";
                                //SharedMemoryCommunicator communicator(name);

                                std::string data = std::to_string(delay);
                                std::ostringstream oss;
                                    oss << uInfo;
                                    std::string uInfoStr = oss.str();
                                //communicator.writeData(data);

                                DataStorage::setData("MacDelayUl", uInfoStr);
